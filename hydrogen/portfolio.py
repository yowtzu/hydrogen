...
class Portfolio:
    ...
    def __init__(self, name: str, cache_result=True):
        ...
        self.rules = [  # (EWMAC, {"fast_span": 2, "slow_span": 8} ) ,
    ...
    def _calc_forecast(self, clip=False):
        ...
        for ticker, inst in self.ticker_instrument_map.items():
            forecasts = [ signal_scalar(rule(inst, **kargs)) for rule, kargs in self.rules ]
            if clip:
    ...
    def forecast(self, ticker_list=[]):
        if not self._forecast:
            self._calc_forecast()

        ticker_list = self._all_tickers_if_empty(ticker_list)
        res = { ticker:self._forecast[ticker] for ticker in ticker_list }
        return res
