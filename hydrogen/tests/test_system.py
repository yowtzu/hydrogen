...
class SystemTest(unittest.TestCase):
    ...
    def test_properties(self):
        assert(system.n_day_in_year == 365)
        assert(system.root_n_day_in_year == np.sqrt(365))
        assert(system.n_bday_in_year == 256)
        assert(system.root_n_bday_in_year == np.sqrt(256))
        assert(system.n_month_in_year == 12)
        assert(system.n_bday_in_3m == 64)
        assert(system.n_week_in_year == 52)
        ...
        assert(system.avg_abs_forecast == 10.0)